...
visitors %>% filter(n_2022 == visitors$n_2022 %>% max())

visitors %>% filter(attraction == "National Museum of Scotland")

visitors %>% filter(setting == "O",
...
#gives the datatype of the selected variable in the data frame
```

```{r Q2_part2}
visitors %>% summarise_all(class)
#applies a function (in this case class see above) to all the variables in the data frame
```
...
**Not Applicable, as the values for n_2021 aren't filled in for some attractions so the mean and median functions cant perform their calculations properly**
...
**na.rm = TRUE means that the NA values get removed before calculations are performed **
...
**it suggests that the data is skewed towards larger values, as in there are a few large values and a lot of smaller ones. the averages of the few larger values contribute more to the mean and contribute little to nothing to the median.**
...
free_counts <- visitors_with_nations %>% filter(admission == "Free") %>% count(nation)

nations_with_counts <- visitors_with_nations %>% count(nation) %>%
                        mutate(
                          n_free = case_when(
                            nation == "Wales" ~ 0,
                            nation == "England" ~ free_counts$n[1],
                            nation == "Northern Ireland" ~ free_counts$n[2],
                            nation == "Scotland" ~ free_counts$n[3]
                          )
                        )

nations_with_counts %>%
  mutate(
    ratio_free = case_when(
      nation == "Wales" ~ 0,
      nation == "England" ~ n_free[1]/n[1],
      nation == "Northern Ireland" ~ n_free[2]/n[2],
      nation == "Scotland" ~ n_free[3]/n[3]
    )
  ) %>% select(nation,ratio_free)
...
```{r Ex_n}
# Write your code here
percentage_change <- visitors %>%
                      mutate( p_change = (n_2022/n_2021 - 1)*100) %>% select(admission,region,p_change)

percentage_change %>%
  filter(region == "Scotland") %>%
  group_by(admission) %>%
  summarise(mean_p_change = mean(p_change, na.rm = TRUE))
